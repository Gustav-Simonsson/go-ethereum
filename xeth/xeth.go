...
func (self *XEth) Transact(fromStr, toStr, valueStr, gasStr, gasPriceStr, codeStr string) (string, error) {
	...
	fromPassStr := "" // TODO: trigger native Mist window to get passphrase from user? for now assume we use plain text key store for testing
	...
	tx.SetNonce(nonce)
	// TODO: allow a RPC call to transact when an account is unlocked? (e.g. Sign(...) )
	sig, err := self.accountManager.SignLocked(&accounts.Account{Address: from}, fromPassStr, tx.Hash())
	if err != nil {
		return "", err
	}
