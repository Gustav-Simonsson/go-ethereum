...
func (self *XEth) Transact(fromStr, fromPassStr, toStr, valueStr, gasStr, gasPriceStr, codeStr string) (string, error) {
	...
	tx.SetNonce(nonce)
	sig, err := self.accountManager.Sign(&accounts.Account{Address: from}, fromPassStr, tx.Hash())
	if err != nil {
		return "", err
	}
